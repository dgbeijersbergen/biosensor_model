...
k_off_vals = np.logspace(-6,-1,3)
#k_off_vals = [params.k_off, 0.01*params.k_off]
...
c_in_vals = np.logspace(-7,-3,3) * 1e3  #input concentration [mol/L = M]
# 100 nM - 10 uM - 1 mM
...
H_c_vals = [params.H_c, 100* params.H_c, 0.1*params.H_c]
#H_c_vals = [params.H_c]
#Q_in_uL_min = np.array([50])
Q_in_uL_min = np.logspace(-1,4,5)                # flow rate in uL/min
#Q_in_uL_min = [10*params.Q_in]
b_m_vals = [params.b_m, params.b_m/10]
#b_m_vals = [params.b_m]
...
results = []
total = len(b_m_vals) * len(k_on_vals) * len(k_off_vals) * len(c_in_vals) * len(D_vals) * len(H_c_vals) * len(Q_in_vals) * len(V_in_vals) # number of combinations

# loop over parameters and calculate results
for k_on, k_off, c_in, Q_in, D, H_c, V_in, b_m in tqdm(itertools.product(k_on_vals, k_off_vals, c_in_vals, Q_in_vals, D_vals, H_c_vals, V_in_vals, b_m_vals),total=total,desc="Running simulations"):
    params.k_on = k_on
    params.k_off = k_off
    params.c_in = c_in
    params.Q_in = Q_in
