from biosensor.model.calculate_Sherwood import *

# computes depletion layer height delta
def compute_delta(Q_in, params, sharpness):
    if Q_in <= 0:
        return params.H_c

    k_m, F = compute_k_m(Q_in, params, sharpness)

    Pe_H = Q_in / (params.D * params.W_c)

    # complete delivery
    if F >= Pe_H:
        delta = params.H_c
    else:
        delta = params.L_s / F

    return min(delta, params.H_c)

# system of non-dimensional ODEs
def ode_binding_hat(t_hat, y, params):
    b_hat, c_hat, c_s_hat, N_out_hat1, N_out_hat2 = y

    # --- parameters ---
    W_c, L_c, H_c = params.W_c, params.L_c, params.H_c
    k_on, k_off = params.k_on, params.k_off
    b_m, L_s, W_s = params.b_m, params.L_s, params.W_s
    c_in, V_in, Q_in = params.c_in, params.V_in, params.Q_in
    flow_off = params.flow_off

    # --- geometry ---
    S = L_s * W_s
    V = W_c * L_c * H_c
    params.V = V

    # --- dimensionless groups ---
    tau = V / Q_in
    gamma = (S * b_m) / (V * c_in)
    t_pulse_hat = V_in / V

    # --- injection switch ---
    injecting = (t_hat < t_pulse_hat)
    if injecting == True:
        H = 1.0
        Q_eff = Q_in
    else:
        H = 0.0
        Q_eff = 0.0

    if Q_eff > 0:
        k_m = params._k_m_cached
    else:
        k_m = 0.0

    # --- depletion layer height ---
    delta = params.delta if (injecting or not flow_off) else params.H_c

    V_s = W_s * L_s * delta
    V_b = V - V_s

    # --- binding kinetics ---
    db_hat_dt = tau * (k_on * c_in * c_s_hat * (1 - b_hat) - k_off * b_hat)     # eq. D.8

    # enforce saturation
    b_eq_hat = (k_on * c_in) / (k_on * c_in + k_off)
    if (b_hat >= 1.0 or b_hat > b_eq_hat) and db_hat_dt > 0:
        db_hat_dt = 0.0

    ## --- ODEs --- ##
    if injecting or not flow_off: # flow on (injection or continuous)

        # -- non-interacting concentration c_b --
        if delta == H_c:
           dc_hat_dt = 0
           dNouthat1_dt = 0
        else:
            dc_hat_dt = tau / V_b * (Q_eff - k_m * W_c * H_c) * (H - c_hat) # eq. D.7
            dNouthat1_dt = tau / V_b * (Q_eff - k_m * W_c * H_c) * (c_hat)

        # -- interfacial concentration c_s --
        dcs_hat_dt = (tau * k_m * H_c) / (delta * L_s) * (1 - c_s_hat) - (tau/ delta) *  (k_on * c_s_hat * b_m * (1 - b_hat) - (k_off * b_m * b_hat) / c_in)  # eq. D.6
        dNouthat2_dt = (tau * k_m * H_c) / (delta * L_s) * c_s_hat

    else:  # flow off, post injection
        dcs_hat_dt = -(gamma * H_c / delta) * db_hat_dt
        dc_hat_dt = 0.0
        dNouthat1_dt = 0.0
        dNouthat2_dt = 0.0

    return [
        db_hat_dt,
        dc_hat_dt,
        dcs_hat_dt,
        dNouthat1_dt,
        dNouthat2_dt,
    ]