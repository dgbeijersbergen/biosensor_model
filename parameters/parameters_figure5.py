...
params = ModelParams(
    ...
    W_c = 1e-4,    # channel width [m]
    L_c = 1e-5,   # channel length [m]
    H_c = 1e-4,    # channel height [m]

    # molecular parameters
    D = 1e-11,      # diffusion coefficient EpCAM [m^2/s]
    ...
    # sensor parameters
    k_on = 1.8e4,        # association constant [mol m^-3 s^-1]
    k_off = 1e-3,   # disassociation constant [1/s]
    b_m = 1e-7,          # max binding density [mol/m^2]
    L_s = 1e-5,    # sensor length [m]     # circular: 0.9571e-2,    # sensor length [m]
    W_s = 1e-4,    # sensor width [m]

    # input parameters
    c_0 = 0,        # initial bulk concentration [mol/m^3]
    #c_in = 100e-6,      # input concentration [mol/m^3] (1 M = 1e3 mol/m^3)
    c_in = 1e-9,      # input concentration [mol/L] (1 mol/m^3 = 1 mM)
    V_in = 1000*1e-9,      # volume input [m^3]   (1uL = 1e-9 m^3)
    Q_in = 0.01,     # volume flow rate [uL/min]
