...
params = ModelParams(
    ...
    # sensor parameters
    k_on = 1e10,        # association constant [mol m^-3 s^-1]
    k_off = 1e-1,   # disassociation constant [1/s]
    #b_m = 4e-8,          # max binding density [mol/m^2]
    ...
    c_in = 10,   # input concentration [mol/m^3] (1 M = 1e3 mol/m^3)
    V_in = 200e-9,      # volume input [m^3]   (1uL = 1e-9 m^3)
